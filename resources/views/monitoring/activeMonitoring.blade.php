{{-- resources/views/active-rentals/index.blade.php --}}
@extends('layout.app')
...
@section('content')
            ...
            <button onclick="openRentalModal({{ $rental['id'] }})"
                ...
                <div class="p-4 space-y-3">
                    <div>
                        ...
                        <p class="font-semibold text-gray-900">{{ $toolNames }}</p>
                    </div>
                    ...
                    <div class="pt-2 border-t">
                        ...
                        <p class="text-lg font-bold text-blue-600">${{ number_format($rental['totalRevenue'], 2) }}</p>
                    </div>
                </div>
            </button>
        @empty
    ...
    <script>
        ...
        const statusList = ['On Track', 'Delivered', 'Pending', 'Menunggak', 'Returning', 'On Check'];
        ...
        function openRentalModal(rentalId) {
            ...
            const totalRevenue = rental.totalRevenue;
            const dailyAvg = rental.dailyAverage;
            ...
            const deliveryHtml = rental.driverName ?
                `<div class="bg-green-50 border border-green-200 rounded-lg p-4">
                   ...
                   <div class="grid grid-cols-2 gap-4">
                       <div><p class="text-gray-600 text-sm">Driver</p><p class="font-semibold text-gray-900">${rental.driverName}</p></div>
                       <div><p class="text-gray-600 text-sm">Location</p><p class="font-semibold text-gray-900">${rental.deliveryLocation ?? '-'}</p></div>
                       ${rental.estimatedDeliveryTime
                           ? `<div class="col-span-2"><p class="text-gray-600 text-sm">Estimated Delivery</p><p class="font-semibold text-gray-900">${rental.estimatedDeliveryTime}</p></div>`
                           : ''}
                   </div>
               </div>` :
            ...
            // Items list
            const itemsHtml = rental.items.map((item, idx) => `
        ...
        `).join('');
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4 pb-4 border-b">
                    <div><p class="text-gray-600 text-sm">Invoice Number</p><p class="text-lg font-semibold text-gray-900">${rental.invoiceNumber}</p></div>
                    <div><p class="text-gray-600 text-sm">Status</p><span class="inline-block px-3 py-1 rounded-full text-sm font-semibold ${getStatusBadgeColor(rental.rentalStatus)}">${rental.rentalStatus}</span></div>
                    <div><p class="text-gray-600 text-sm">Customer</p><p class="text-lg font-semibold text-gray-900">${rental.customerName}</p></div>
                    <div><p class="text-gray-600 text-sm">Created</p><p class="text-lg font-semibold text-gray-900">${fmtDate(rental.createdDate)}</p></div>
                </div>

                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 class="font-semibold text-gray-900 mb-3">Rental Timeline</h4>
                    <div class="grid grid-cols-3 gap-4">
                        <div><p class="text-gray-600 text-sm">Start Date</p><p class="font-semibold text-gray-900">${fmtDate(rental.rentalStartDate)}</p></div>
                        <div><p class="text-gray-600 text-sm">End Date</p><p class="font-semibold text-gray-900">${fmtDate(rental.rentalEndDate)}</p></div>
                        <div><p class="text-gray-600 text-sm">Days Remaining</p><p class="font-bold text-lg ${daysColor}">${days} days</p></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-0 border rounded-lg overflow-hidden">${itemsHtml}</div>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <p class="text-gray-600 text-sm">Total Revenue</p>
                        <p class="text-2xl font-bold text-green-700">${fmt(totalRevenue)}</p>
                    </div>
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p class="text-gray-600 text-sm">Daily Average</p>
                        <p class="text-2xl font-bold text-blue-700">${fmt(dailyAvg)}</p>
                    </div>
                </div>
        ...
        function updateRentalStatus(rentalId, newStatus) {
            // Kirim update ke server via fetch (AJAX)
            fetch(`/active-rentals/${rentalId}/status`, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Re-render tombol status di modal
                        const statusButtons = statusList.map(status => `
                ...
                `).join('');
                        document.getElementById('status-buttons').innerHTML = statusButtons;

                        // Update badge di modal header
                        ...
                        // Update card di halaman (reload halaman untuk sinkron)

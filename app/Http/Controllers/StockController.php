...
class StockController extends Controller
{
    ...
    public function overview(Request $request)
    {
        ...
        $filteredStock = $warehouseFilter ? array_values(array_filter($stockOverview, fn($s) => $s['warehouseName'] === $warehouseFilter)) : $stockOverview;

        // Hitung summary
        $totalItems = array_sum(array_column($filteredStock, 'quantity'));
        $totalTools = count($stockOverview);
        $lowStockCount = count(array_filter($filteredStock, fn($s) => $s['quantity'] < 3));
    ...
    public function movement(Request $request)
    {
        ...
        $filtered = $typeFilter
            ? array_values(array_filter($movements, fn($m) => $m['type'] === $typeFilter))
            : $movements;
 
        $types = ['IN', 'OUT', 'RENT', 'RETURN', 'LOST', 'DAMAGED'];
